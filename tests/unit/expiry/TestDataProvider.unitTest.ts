...
describe('TestDataProvider', () => {
  ...
  context('for getTestHistory', () => {
    ...
    describe('when getBySystemNumber returns test results with invalid testEndTimestamp', () => {
      it('should return the filtered data', async () => {
        ...
        expect(result[0].testEndTimestamp).toBe('2019-03');
      });
    });
    describe('when getBySystemNumber returns invalid testStartTimestamp', () => {
      it('should return the filtered data', async () => {
        ...
        const result = await testDataProvider.getTestHistory('123');
        expect(result).toHaveLength(1);
        expect(result[0].testStartTimestamp).toBe('2019-03');
      });
    });
            ...
            Promise.resolve([
              {
                ...
                testTypes: [{ testCode: 'aat1', testExpiryDate: '2020-01-05' }],
                testStatus: enums.TEST_STATUS.SUBMITTED,
              },
              {
                testStartTimestamp: '2019-03-10T08:47:59.269Z',
                testEndTimestamp: '2019-03-10T09:30:59.269Z',
                testTypes: [{ testCode: 'fft1', testExpiryDate: '2020-02-15' }],
                testStatus: enums.TEST_STATUS.SUBMITTED,
              },
              {
                testStartTimestamp: '2019-01-10T08:47:59.269Z',
                testEndTimestamp: '2019-01-10T09:30:59.269Z',
                testTypes: [{ testCode: 'rpt1', testExpiryDate: '2020-01-04' }],
                testStatus: enums.TEST_STATUS.SUBMITTED,
              },
            ]),
        }));
          ...
          getBySystemNumber: () =>
            Promise.resolve([
              ...
              },
              {
                testStartTimestamp: '2019-03-10T08:47:59.269Z',
                testEndTimestamp: '2019-03-10T09:30:59.269Z',
                testTypes: [{ testCode: 'fft1', testExpiryDate: undefined }],
                testStatus: enums.TEST_STATUS.SUBMITTED,
              },
              {
                testStartTimestamp: '2019-01-10T08:47:59.269Z',
                testEndTimestamp: '2019-01-10T09:30:59.269Z',
                testTypes: [{ testCode: 'rpt1', testExpiryDate: '2020-01' }],
                testStatus: enums.TEST_STATUS.SUBMITTED,
              },
            ]),
        }));
  ...
  context('for insertTestResult', () => {
    it("should call the 'testResultDAO.createSingle' method and 'logDefectsReporting'", async () => {
      testDataProvider = new TestDataProvider();
      MockTestResultsDAO = jest.fn().mockImplementation(() => ({
        createSingle: (something: any) => Promise.resolve({}),
      }));
      testDataProvider.testResultsDAO = new MockTestResultsDAO();
  ...
  context('for updateTestResult', () => {
    it('should update the test code, the test name and the name', async () => {
      ...
      testDataProvider.testResultsDAO.getTestCodesAndClassificationFromTestTypes =
        jest.fn(() =>
          Promise.resolve({
            defaultTestCode: 'foo',
            linkedTestCode: 'bar',
      ...
      expect(output[0].testTypeClassification).toBe('foobar');
      expect(output[0].name).toBe('baz');
      expect(output[0].testTypeName).toBe('barfoo');
    });
  });
});
