...
describe("ValidationUtil", () => {
  context(
    "calling fieldsNullWhenDeficiencyCategoryIsOtherThanAdvisoryresultsPayload having defects other than advisory",
    () => {
      it("should add missing fields to defects", () => {
        const mockData = testResultsMockDB[4];

        const result = ValidationUtil.fieldsNullWhenDeficiencyCategoryIsOtherThanAdvisory(
          mockData
        );
  ...
  context(
    "calling reasonForAbandoningPresentOnAllAbandonedTests with abandoned testTypes",
    () => {
      it("should return whether all have reasonForAbandoning or not", () => {
        const mockData = testResultsMockDB[5];
        const result = ValidationUtil.reasonForAbandoningPresentOnAllAbandonedTests(
          mockData
        );
