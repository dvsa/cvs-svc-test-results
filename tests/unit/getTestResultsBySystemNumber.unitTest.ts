...
describe('getTestResultBySystemNumber', () => {
      ...
      return testResultsService
        ...
        .then((returnedRecords: any) => {
          ...
          expect(JSON.stringify(returnedRecords[0])).toEqual(
            JSON.stringify(testResultsMockDB[0]),
          );
  ...
  context('when the testResultId is passed', () => {
    context('and testVersion query param is archived', () => {
      it('should return the testHistory array of a test-result', () => {
        ...
        MockTestResultsDAO = jest.fn().mockImplementation(() => ({
          getBySystemNumber: () => Promise.resolve(Array.of(testResult)),
        }));
        testResultsService = new TestResultsService(new MockTestResultsDAO());
        ...
        return testResultsService
          ...
          .then((returnedRecords: any) => {
            ...
            expect(JSON.stringify(returnedRecords[0])).toEqual(
              JSON.stringify(testResultsMockDB[1]),
            );
            expect(JSON.stringify(returnedRecords[1])).toEqual(
              JSON.stringify(testResultsMockDB[2]),
            );
          });
      });
    });
    context('and testVersion query param is current', () => {
      it('should return the test-result without the testHistory array', () => {
        ...
        testResult.testHistory = [
          'some archived test-result',
          'another archived test-result',
        ];
        MockTestResultsDAO = jest.fn().mockImplementation(() => ({
          getBySystemNumber: () => Promise.resolve(Array.of(testResult)),
        }));
  ...
  context(
    'when a record is found filtered for systemNumber with LEC test Type',
    () => {
      it('should return a populated response with new fields populated with non-null values and status code 200', () => {
        MockTestResultsDAO = jest.fn().mockImplementation(() => ({
          getBySystemNumber: () =>
            Promise.resolve(Array.of(testResultsMockDB[19])),
        }));
        ...
        return testResultsService
          .getTestResultBySystemNumber({
            systemNumber: '1130',
            status: 'submitted',
            fromDateTime: new Date('2017-01-01'),
            toDateTime: new Date(),
          })
          .then((returnedRecords: any) => {
            expect(returnedRecords).toHaveLength(1);
            expect(returnedRecords[0].systemNumber).toBe('1130');
            expect(returnedRecords[0].testTypes[0].testExpiryDate).toBe(
              '2021-10-24',
            );
            expect(returnedRecords[0].testTypes[0].emissionStandard).toBe(
              '0.08 g/kWh Euro 3 PM',
            );
          });
      });
    },
