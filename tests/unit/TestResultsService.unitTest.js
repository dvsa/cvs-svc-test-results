...
context('setAnniversaryDate', () => {
  context('if testResult is hgv or trl', () => {
  ...
  context('if testResult is psv', () => {
    ...
    it('should set anniversary date the same as expiryDate', () => {
      ...
      expect(testResultWithAnniversaryDate.testTypes[0].testAnniversaryDate).not.be.eql(undefined)
      expect(testResultWithAnniversaryDate.testTypes[0].testAnniversaryDate).to.be.eql(dateFns.addDays(dateFns.subMonths(testResultWithAnniversaryDate.testTypes[0].testExpiryDate, 2), 1).toISOString())
    })
  })
})
