...
describe('insertTestResult', () => {
  ...
  context('when inserting a testResult that has an ADR testType with expiryDate and certificateNumber', () => {
    it('should not throw error', () => {
      ...
      const testResultsService = new TestResultsService(testResultsDAOMock)
      testResultsDAOMock.testNumber = { testNumber: 'W01A00209', id: 'W01', certLetter: 'A', sequenceNumber: '002' }
      let testResultWithAdrTestType = [...postObject][6]
  ...
  context('when inserting a testResult that has an ADR testType without expiryDate', () => {
    it('should throw 400 and descriptive error message', () => {
      ...
      const testResultsService = new TestResultsService(testResultsDAOMock)
      testResultsDAOMock.testNumber = { testNumber: 'W01A00209', id: 'W01', certLetter: 'A', sequenceNumber: '002' }
      let testResultWithAdrTestTypeWithoutExpiryDate = [...postObject][6]
    ...
    it('should throw 400 and descriptive error message', () => {
      ...
      return testResultsService.insertTestResult(testResultWithAdrTestTypeWithoutExpiryDate)
        ...
        .catch((error) => {
          expect(error).to.be.instanceof(HTTPError)
          expect(error.statusCode).to.be.eql(400)
          expect(error.body).to.be.eql('Certificate number not present on LEC or ADR test type')
        })
    })
  })
...
describe('isTestTypeAdr', () => {
  const testResultsDAOMock = new TestResultsDAOMock()
  context('testType that is adr', () => {
    const testTypeAdr = JSON.parse('{\n' +
    ...
    it('should return true', () => {
      const testResultsService = new TestResultsService(testResultsDAOMock)
      expect(testResultsService.isTestTypeAdr(testTypeAdr)).to.eql(true)
    })
  })
})
describe('adrTestTypeWithoutExpiryDate', () => {
  const testResultsDAOMock = new TestResultsDAOMock()
  context('when payload contains at least one adr testType which does not have expiry date', () => {
    const testResultAdrWithoutExpiryDate = JSON.parse('{\n' +
        '    "testerStaffId": "1",\n' +
        '    "testResultId": "1115",\n' +
    ...
    it('should return true', () => {
      const testResultsService = new TestResultsService(testResultsDAOMock)
      expect(testResultsService.adrTestTypeWithoutExpiryDate(testResultAdrWithoutExpiryDate)).to.eql(true)
    })
  })
})
