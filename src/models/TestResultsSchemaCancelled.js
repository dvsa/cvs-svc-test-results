...
const defectsSchema = Joi.object().keys({
  ...
  additionalInformation: Joi.object().keys({
    ...
    notes: Joi.string().max(500).required().allow(null).allow('')
  }),
  ...
  deficiencyText: Joi.string().required().allow(null).allow(''),
...
const testTypesSchema = Joi.object().keys({
  ...
  certificateNumber: Joi.string().required().allow(null).allow(''),
  ...
  reasonForAbandoning: Joi.string().min(1).max(500).required().allow(null).allow(''),
  additionalNotesRecorded: Joi.string().min(1).max(500).required().allow(null).allow(''),
  additionalCommentsForAbandon: Joi.string().min(1).max(500).required().allow(null).allow(''),
...
const testResultsSchema = Joi.object().keys({
  ...
  testerName: Joi.string().min(1).max(60).required().allow(''),
  testerEmailAddress: Joi.string().min(1).max(60).required().allow(''),
  testerStaffId: Joi.string().min(1).max(9).required().allow(''),
  ...
  testTypes: Joi.array().items(testTypesSchema).required().allow(''),
