...
export class ExpiryDateStrategyFactory {
  ...
  public static GetExpiryStrategy(testTypeForExpiry: TestTypeForExpiry, dateProvider: DateProvider): IExpiryDateStrategy {
    let strategiesFound = 0;
    let selectedStrategy: IExpiryDateStrategy = new NoImplementationStrategy(testTypeForExpiry, dateProvider);
    const vehicleConfig = ExpiryDateStrategyFactory.getStrategyMapping(testTypeForExpiry.vehicleType, expiryMapping);
  ...
  private static createStrategy(strategy: string, testTypeForExpiry: TestTypeForExpiry, dateProvider: DateProvider): IExpiryDateStrategy {
    switch (strategy) {
      ...
      case EXPIRY_STRATEGY.PSV_REGN_ANNIVERSARY:
        return new PsvRegistrationAnniversaryStrategy(testTypeForExpiry, dateProvider);
      ...
      case EXPIRY_STRATEGY.HGV_TRL_MOST_RECENT:
        return new HgvTrlMostRecentExpiryStrategy(testTypeForExpiry, dateProvider);
      default:
  ...
  private static getStrategyMapping(vehicleType: VEHICLE_TYPE, expiryConfig: any) {
